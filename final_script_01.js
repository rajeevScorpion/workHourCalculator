document.addEventListener('DOMContentLoaded', (event) => {
    ...
    calculatorForm.addEventListener('submit', function(e) {
        ...
        // Scenario 3: User provides in time and surplus/shortfall
            ...
            const surplusShortfallInSeconds = parseFloat(surplusShortfallField.value) * 3600;

            let effectiveWorkTimeInSeconds;

            if (surplusShortfallType === 'surplus') {
                effectiveWorkTimeInSeconds = WORK_HOURS_IN_SECONDS - surplusShortfallInSeconds;
            } else {
                effectiveWorkTimeInSeconds = WORK_HOURS_IN_SECONDS + surplusShortfallInSeconds;
            }

            const outTimeInSeconds = inTimeInSeconds + effectiveWorkTimeInSeconds;

            result = `You can leave at ${convertToHHMMSS(outTimeInSeconds)}`;
            result += `<br>Effective Work Time: ${convertToHHMMSS(effectiveWorkTimeInSeconds)}`;
        }
